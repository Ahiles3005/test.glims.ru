...
class DPD
{
	...
	public function Calculate($profile, $arConfig, $arOrder, $STEP, $TEMP = false)
	{
		try {
			...
			foreach($event->getResults() as $eventResult) {
				...
				$result = array_merge($result, (array) $eventResult->getParameters());
			}
	...
	protected static function makeShipment($arOrder = false)
	{
		if (!self::$shipment || $arOrder) {
			$senderLocation = Utils::getSaleLocationId();

			if (($senderIndex = Utils::getSenderIndexByLocation($arOrder['LOCATION_TO'])) !== false) {
				$senders        = Utils::getSenders();
				$senderLocation = $senders[$senderIndex]['LOCATION'];
			}

			self::$shipment = new Shipment();
			self::$shipment
			    ->setSender($senderLocation)
	 ...
	 * @return string
	 */
	protected static function getHiddenHTML($shipment, $tariff, $profile, $selected = false)
	{
		if (!self::needIncludeComponent() || !$shipment) {
			return '';
		}
		...
		$html = $html . sprintf(''
			. '<input type="hidden" id="IPOLH_DPD_TERMINAL" name="IPOLH_DPD_TERMINAL['. $profile .']" value="'. $terminalCode[$profile] .'">'
			...
			. ($selected ? '<iframe src="about:blank" onload="BX.onCustomEvent(BX(\'DPD_pvz\'), \'dpdPickup:selectTerminal\', [\''. $terminalCode[$profile] .'\', false]);" style="display: none"></iframe>' : '')
			. '<br>'
			. '<a class="DPD_openTerminalSelect btn btn-default btn-md" data-component-params="%s" data-component-result="%s">%s</a>'
			. '<br><br><div type="" id="IPOLH_DPD_TERMINAL_ADDR">' . $terminalAddr . '</div>'
		,
	...
	public static function OnSalePaySystemFilter(&$arResult, $arUserResult, $arParams)
	{
		...
		$profile  = static::getDeliveryProfile($arUserResult['DELIVERY_ID']);
		$profiles = [static::COURIER_PROFILE_ID, static::PICKUP_PROFILE_ID];
		...
		if ($profile == static::COURIER_PROFILE_ID && $shipment->getReceiver()['IS_CASH_PAY'] == 'Y') {
			return;
		} elseif ($profile == static::PICKUP_PROFILE_ID && $shipment->isPossibileSelfDelivery()) { 
			// если в городе есть терминалы которые принимаю НПП
			$terminalCode = !empty($_REQUEST['IPOLH_DPD_TERMINAL']['PICKUP'])
				? $_REQUEST['IPOLH_DPD_TERMINAL']['PICKUP']
				: false
			;

			$terminalCode = !empty($_REQUEST['order']['IPOLH_DPD_TERMINAL']['PICKUP'])
				? $_REQUEST['order']['IPOLH_DPD_TERMINAL']['PICKUP']
				: false
			;

			$terminal = $terminalCode
				? \Ipolh\DPD\DB\Terminal\Model::getByCode($terminalCode)
				: false
			;

			if (!$terminal || $terminal->checkShipmentPayment($shipment)) {
				return;
			}
		}

		$stPaymentIds = Option::get(IPOLH_DPD_MODULE, 'COMMISSION_NPP_PAYMENT_'. $arUserResult['PERSON_TYPE_ID'] .'_'. SITE_ID);
	...
	public static function OnSaleDeliveryHiddenHTML(&$arResult, $arUserResult, $arParams)
	{
		foreach ($arResult['DELIVERY'] as $deliveryId => &$arDelivery) {
			if (!static::isSelfInstance($deliveryId)) {
				continue;
			}

			list($code, $profile) = explode(':', static::getDeliveryCode($arDelivery['ID']));
			$data = isset(static::$lastResults[$profile]) ? static::$lastResults[$profile] : false;

			if ($data) {
				$arDelivery['PERIOD_TEXT'] .= static::getHiddenHTML(
					$data['SHIPMENT'],
					$data['TARIFF'],
					$data['PROFILE'],
