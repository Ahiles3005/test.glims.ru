					...
					<table class="contacts-stores no-border shops list">
						<?foreach($arResult['SECTIONS'] as $si => $arSection):?>
							...
							<?foreach($arSection['ITEMS'] as $i => $arItem):?>
								...
								<tr class="item" id="<?=$this->GetEditAreaId($arItem['ID'])?>" data-store="">
									<?if($imageSrc):?>
										<td class="hidden-xs img">
											<a href="<?=$arItem["DETAIL_PAGE_URL"];?>" <?=$arItem["TARGET"];?>>
									...
									<td class="hidden-xs" <?=(($arResult['ITEMS_HAS_IMG'] && !$imageSrc) ? 'colspan=2' : '');?>>
										<div class="title">
											<a href="<?=$arItem["DETAIL_PAGE_URL"];?>" <?=$arItem["TARGET"];?> class="dark_link"><?=$arItem['NAME'];?><?=$address;?></a>
										</div>
									...
									<td class="phone hidden-xs phone-space">
                                    ...
                                    <? if ($arItem['PROPERTIES']['SITE']['VALUE']): ?>
                                        <noindex>
                                            <a href="//<?= $arItem['PROPERTIES']['SITE']['VALUE'] ?>"
                                               class="black" target="_blank" rel="nofollow"><?= $arItem['PROPERTIES']['SITE']['VALUE'] ?></a>
                                        </noindex>
                                    <? endif; ?>
									</td>
