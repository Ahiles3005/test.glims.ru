...
<?php
$db_props = CIBlockElement::GetProperty($arResult["IBLOCK_ID"], $arResult["ID"], array("sort" => "asc"), Array("CODE"=>"WORK_ORDER"));
if($ar_props = $db_props->Fetch()) {
    $WORK_ORDER = $ar_props["VALUE"]["TEXT"];
    $bShowDetailOrderTab = true;
}else {
    $WORK_ORDER = false;
    $bShowDetailOrderTab = false;
}
?>
	...
	<?if($templateData['LINK_SERVICES']):?>
		...
		if($htmlBuyServicesAnounce && trim($htmlBuyServicesAnounce) && strpos($htmlBuyServicesAnounce, 'error') === false){?>
			...
			<?$htmlBuyServicesAnounceBefore = '<div class="js-services-in-side"><div class=\' buy_services_wrap in_announce \' data-parent_product='.$arResult["ID"].'>';?>
				<?
				...
				<div class="ordered-block js-store-scroll tabs-block">
						...
						<div class="tabs arrow_scroll">
							<ul class="nav nav-tabs font_upper_md">
								...
								<?foreach($arTabOrder as $value):?>
                                    ...
                                    <?//show order block?>
                                    <?if($value == "order"):?>
                                        <?if($bShowDetailOrderTab && ($arParams["PROPERTIES_DISPLAY_LOCATION"] != "TAB")):?>
                                            <li class="bordered rounded3 <?=(!($iTab++) ? 'active' : '')?>"><a href="#order" data-toggle="tab">Порядок работ</a></li>
                                        <?endif;?>
                                    <?endif;?>
									<?//show char block?>
									<?if($value == "char"):?>
					...
					<div class="tab-content<?=($i <= 1 ? ' not_tabs' : '')?>">
						...
						<?foreach($arTabOrder as $value):?>
                            ...
                            <?//order?>
                            <?if($value == "order"):?>
                                <?php if (!empty($WORK_ORDER) && $bShowDetailOrderTab):?>
									<?
									global $arRegion;
									if ($arRegion['PROPERTY_DEFAULT_VALUE'] !== 'Y'):?>
												<!--noindex-->
									<?endif?>
                                    <div class="tab-pane <?=$value;?> <?=(!($iTab++) ? 'active' : '')?>" id="order">
                                        <?=$WORK_ORDER?>
                                    </div>
										<?if ($arRegion['PROPERTY_DEFAULT_VALUE'] !== 'Y'):?>
											<!--/noindex-->
							...
							<?if($value == "buy_services"):?>
								<?if($bShowBuyServicesTab):?>
									<div class="tab-pane <?=$value;?> <?=(!($iTab++) ? 'active' : '')?>" id="buy_services">
										...
										<?endif;?>
										<div class='buy_services_wrap js-scroll-services' data-parent_product=<?=$arResult['ID']?>>
											<?=$htmlBuyServices;?>
										</div>
									</div>
								<?endif;?>
