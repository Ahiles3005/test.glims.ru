...
if($arResult['ITEMS'])
{
	...
	foreach($arResult['ITEMS'] as $key_item => $arItem)
	{
	    if ($arItem['PROPERTIES']['MAP']['VALUE']) {
            ...
            $arResult['COORDINATES'][] = [(float)$coords[0], (float)$coords[1]];
            $arResult['STORES_NAME'][] = $arItem['NAME'];
            $arResult['STORES_DETAIL'][] = $arItem['DETAIL_PAGE_URL'];
        }
		...
		if ($arRegion['PROPERTY_DEFAULT_VALUE'] !== 'Y' ) {
			$arResult['ITEMS'][$key_item]['DETAIL_PAGE_URL'] = '//'.$defaultRegion . $arItem['DETAIL_PAGE_URL'];
			$arResult['ITEMS'][$key_item]['TARGET'] = 'target="_blank"';
		}
	}
    $arResult['COORDINATES'] = CUtil::PhpToJSObject($arResult['COORDINATES']);
    $arResult['STORES_NAME'] = CUtil::PhpToJSObject($arResult['STORES_NAME']);
    $arResult['STORES_DETAIL'] = CUtil::PhpToJSObject($arResult['STORES_DETAIL']);
