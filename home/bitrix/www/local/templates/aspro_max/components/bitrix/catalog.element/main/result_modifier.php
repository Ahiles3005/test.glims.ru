...
if(!empty($arResult['DISPLAY_PROPERTIES']['PRODUCT_VARIANTS'])){
	
	unset($arResult['DISPLAY_PROPERTIES']['PRODUCT_VARIANTS']);

} 
if(!empty($arResult['PROPERTIES']['PRODUCT_VARIANTS']['VALUE'])){


	$arSelect = Array("ID", "NAME", "DATE_ACTIVE_FROM", 'DETAIL_PAGE_URL', 'PROPERTY_168');
	$arFilter = Array("IBLOCK_ID"=>$arParams['IBLOCK_ID'], "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y", "ID"=>$arResult['PROPERTIES']['PRODUCT_VARIANTS']['VALUE']);
	$res = CIBlockElement::GetList(Array("SORT"=>"ASC"), $arFilter, false, Array("nPageSize"=>50), $arSelect);
	while($ob = $res->GetNextElement())
	{
		$arFields = $ob->GetFields();
		$arResult['PROPERTIES']['PRODUCT_VARIANTS']['ITEMS'][] = $arFields;
	...
	$arSelect = Array("ID", "NAME", "DATE_ACTIVE_FROM", 'DETAIL_PAGE_URL', 'PROPERTY_PRODUCT_VARIANTS');
	$arFilter = Array("IBLOCK_ID" => $arParams['IBLOCK_ID'], "ACTIVE_DATE"=>"Y",
	"ACTIVE"=>"Y", "=PROPERTY_PRODUCT_VARIANTS"=>$arResult['ID']);
	$res = CIBlockElement::GetList(Array("SORT"=>"ASC"), $arFilter, false, Array("nPageSize"=>50), $arSelect);
	while($ob = $res->GetNextElement())
	{
		$arFields = $ob->GetFields();
		$arResult['PROPERTIES']['PRODUCT_VARIANTS']['MANE_ITEM'] = $arFields;  
	 
	}
	if(!empty($arResult['PROPERTIES']['PRODUCT_VARIANTS']['MANE_ITEM']['ID'])){
		$res = CIBlockElement::GetProperty($arParams['IBLOCK_ID'], 
		$arResult['PROPERTIES']['PRODUCT_VARIANTS']['MANE_ITEM']['ID'], "sort", "asc", array("CODE" => "PRODUCT_VARIANTS"));
		while ($ob = $res->GetNext())
		{
			$arResult['PROPERTIES']['PRODUCT_VARIANTS']['VALUE'][] = $ob['VALUE'];
		}
		if(!empty($arResult['PROPERTIES']['PRODUCT_VARIANTS']['VALUE'])){
 	...
 	// echo '<pre style="display:none;">';
		// print_r( $arResult['PROPERTIES']['PRODUCT_VARIANTS']['VALUE']);
		// print_r( $arResult['PROPERTIES']['PRODUCT_VARIANTS']['ITEMS']);
