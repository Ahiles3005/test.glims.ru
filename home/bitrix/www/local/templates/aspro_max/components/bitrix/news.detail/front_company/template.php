		...
		<div class="item-views company lazy <?=$lightText ? 'company_light_text' : '';?> <?=$arParams['TYPE_IMG'];?> <?=$arParams['TYPE_BLOCK'];?><?=($arParams['TYPE_IMG'] == 'lg' ? ' ' : ' with-padding');?>" <?if($arResult['BG_IMG']):?>data-src="<?=$arResult['BG_IMG'];?>" style="background-image:url(<?=\Aspro\Functions\CAsproMax::showBlankImg($arResult['BG_IMG']);?>)"<?endif;?>>
			<div class="company-block">
				<div class="row">
					<div class="col-md-12 image-block col-xs-12">
						<div class="item video-block">
							...
							<?elseif($bImage):?>
								<div class="image lazy" data-src="<?=$imageSrc;?>" style="background-image:url(<?=\Aspro\Functions\CAsproMax::showBlankImg($imageSrc);?>)"  data-video_source="<?=$videoSource?>" <?=(strlen($videoPlayer) ? ' data-video_player="'.$videoPlayer.'"' : '')?><?=(strlen($videoPlayerSrc) ? ' data-video_src="'.$videoPlayerSrc.'"' : '')?>>
									<?if($bVideo):?>
										<div class="play">
				...
				<div class="row flexbox<?=($arParams['TYPE_BLOCK'] == 'type2' ? ($arParams['REVERCE_IMG_BLOCK'] == 'Y' ? '' : ' flex-direction-row-reverse') : ($arParams['REVERCE_IMG_BLOCK'] == 'Y' ? ' flex-direction-row-reverse' : ''))?>">
					<div class="col-md-<?=($arParams['TYPE_BLOCK'] == 'type2' ? ($arParams['TYPE_IMG'] == 'md' ? 12 : 9) : 6);?> text-block col-xs-12">
						<div class="item">
