					...
					<div class="d-inline landings-list__info-wrapper <?=($bWithHidden ? 'last' : '');?> flexbox flexbox--row <?=($bSlider ? 'with-slider' : 'flexbox--wrap');?>">
							...
							<div class="landings-list__item font_xs  <?=$class?>">
								<div>
									<?if(strlen($url)):?>
										...
										<?else:?>
											<a class="1<?=$url?> landings-list__name<?=($bFilled ? ' landings-list__item--filled-bg box-shadow-sm' : ' landings-list__item--hover-bg');?> rounded3" href="<?=$url?>"><span><?=$arItem['NAME']?></span></a>
										<?endif;?>
									<?else:?>
										<span class="2<?=$url?> landings-list__name<?=($bFilled ? ' landings-list__item--filled-bg box-shadow-sm' : ' landings-list__item--hover-bg');?> rounded3"><span><?=$arItem['NAME']?></span></span>
									<?endif?>
								</div>
							</div>
						<?endforeach?>
						<?if($bHidden):?>
							<div class="landings-list__item font_xs">
								<span class="3<?=$url?> landings-list__name landings-list__item--js-more colored_theme_text_with_hover <?=$classOpened;?>" data-opened="<?=$opened;?>" data-visible="<?=$arParams['COUNT'];?>">
