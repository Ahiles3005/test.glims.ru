...
class NetexPriceList
{
    public static function createExportFile()
    {
         ...
         */
        $sections = [];
        $query = SectionTable::getList([
            'filter' => [
                'IBLOCK_ID' => $catalogIblockId,
                'ACTIVE' => 'Y'
            ],
            'select' => [
                'ID', 'IBLOCK_SECTION_ID', 'NAME', 'CODE', 'DEPTH_LEVEL'
            ]
        ]);
        ...
        $query = \CIBlockElement::GetList(
            ...
            false,
            false,
            ['ID', 'NAME', 'IBLOCK_SECTION_ID', 'CODE', 'PROPERTY_CML2_ARTICLE']
        );
        while ($row = $query->Fetch()) {
        ...
        /**
         * Получаем все торговые предложения и записываем в элементы каталога
         */
        $query = \CIBlockElement::GetList(
            [],
            [
                'IBLOCK_ID' => $scuIblockId,
                'ACTIVE' => 'Y',
                'CATALOG_AVAILABLE' => 'Y',
            ],
            false,
            false,
            ['ID', 'NAME', 'PROPERTY_CML2_LINK', 'PROPERTY_CML2_ARTICLE']
        );
        while ($row = $query->Fetch()) {
            if (isset($arElements[$row['PROPERTY_CML2_LINK_VALUE']])) { // Если есть привязка к товару
                if (isset($arElements[$row['PROPERTY_CML2_LINK_VALUE']])) { // Если товар существует
                    $arElements[$row['PROPERTY_CML2_LINK_VALUE']]['OFFERS'][$row['ID']] = $row;
                }
            }
        }

        /**
         * Создаём структуру вложенности товаров в разделы
         */
        foreach ($arElements as $arElement) {
            if (isset($arElement['OFFERS'])) {
                foreach ($arElement['OFFERS'] as $arOffer) {
                    $sections[$arElement['IBLOCK_SECTION_ID']][$arOffer['ID']] = [
                        'ID' => $arOffer['ID'],
                        'NAME' => $arOffer['NAME'],
                        'CODE' => $arElement['CODE'],
                        'IBLOCK_SECTION_ID' => $arElement['IBLOCK_SECTION_ID'],
                        'PROPERTY_CML2_ARTICLE_VALUE' => $arElement['PROPERTY_CML2_ARTICLE_VALUE'],
                        'PRICE' => \CPrice::GetList([], ['PRODUCT_ID' => $arOffer['ID']])->Fetch()['PRICE']
                    ];
                }
            } else {
                $arElement['PRICE'] = \CPrice::GetList([], ['PRODUCT_ID' => $arElement['ID']])->Fetch()['PRICE'];
                $sections[$arElement['IBLOCK_SECTION_ID']][$arElement['ID']] = $arElement;
            }
        }

        /**
         * Из массива содержащего категории и товары
         * создаем иерархию вложенности (глубина 4)
         */
        $k = 4;
        while ($k) {
            foreach ($sections as  $key => &$section) {
                if ($section['DEPTH_LEVEL'] != 1) {
                    $sections[$section['IBLOCK_SECTION_ID']][$section['ID']] = $section;
                }
            }
            $k--;
        } ;
        ...
        /**
         * Удаляем из массива элементы без вложенности
         */
        foreach ($sections as $key => &$section) {
            if (!empty($sections[$key]['IBLOCK_SECTION_ID']))
                unset($sections[$key]);
        }

        $filepath = $_SERVER["DOCUMENT_ROOT"] . '/price_list.xlsx';

        $writer = new \XLSXWriter();

        $sheetHeader = [
            'Прайс лист сгенерирован: ' . date('d.m.Y') => '@',
        ];
        ...
        $writer->writeSheetHeader('Sheet1', $sheetHeader, ['widths' => [10, 75, 10, 10]]);
        $writer->writeSheetRow('Sheet1', ['', '', 'Розница, руб.', 'Опт, руб.'], [ 'font-size' => 10, 'height' => 14, 'border' => 'left,right,top,bottom']);
        $writer->markMergedCell('Sheet1', 0, 0, 0, 3);

        self::createSection($sections, 'https://glims.ru/products/', $writer, $styles);

        if (file_exists($filepath)) {
            unlink($filepath);
        }

        $writer->setTitle('Цены на продукциию компании');
        $writer->setSubject('Прайс-лист');
        $writer->setAuthor('АО «ГЛИМС-Продакшн»');

        $writer->writeToFile($filepath);

        return "\NetexPriceList::createExportFile();";
    }

    /**
     * Рекурсивная функция для записи в файл товаров
     * + получение ссылки на товар рекурсией
     *
     ...
     * @param array $styles
     */
    public static function createSection(array $arrayMulti, $path, $writer, $styles)
    {
        static $count = 1;
        foreach ($arrayMulti as $array) {
            if (isset($array['DEPTH_LEVEL'])) {
                if ($array['DEPTH_LEVEL'] == 1) {
                    ...
                    $writer->markMergedCell('Sheet1', $writer->countSheetRows('Sheet1')-1, 0, $writer->countSheetRows('Sheet1')-1, 3);

                    self::createSection(
                        $array,
                        'https://glims.ru/products/' . $array['CODE'] . '/',
                        $writer,
                        $styles
                    );
                } else {
                    $writer->writeSheetRow(
                        'Sheet1',
                        [
                            str_repeat(' - ', $array['DEPTH_LEVEL']) . $array['NAME'],
                            '',
                            '',
                            ''
                        ],
                ...
                if (is_array($array)) {
                    $writer->writeSheetRow(
                        ...
                        [
                            $count,
                            '=HYPERLINK("https://glims.ru/products/' . $array['CODE'] . '/" , "' . $array['PROPERTY_CML2_ARTICLE_VALUE'] . ' ' . str_replace('"', '\'', $array['NAME']) . '")',
                            $array['PRICE'],
                            ''
                        ],
                        [
